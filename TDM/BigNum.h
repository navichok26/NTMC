...
class BN {
 public:
    ...
    // арифметические операции BN с BN
    BN operator + (const BN&);
    BN&  operator += (const BN&);
    BN operator - (const BN&);
    BN&  operator -= (const BN&);
...
BN BN::gcd(const BN& other) const {
    BN a = *this;
    BN b = other;
    BN zero; zero = 0;
...
void BN::cin_base10() {
    ...
    if (k == 0) {
        cerr << "Ошибка: Неверный ввод. Пустая строка." << endl;
        exit(EXIT_FAILURE);
    }
    ...
    while (j < k) {
        if ('0'  > s[j] || s[j] > '9') {
            cerr << "Ошибка: Ввод содержит недопустимые символы или число отрицательное." << endl;
            exit(EXIT_FAILURE);
        }
